...
for (const themeDir of themesDir) {
  ...
  for (const pagesDir of pagesDirs) {
    ...
    for (const templatesDir of templatesDirs) {
      const ftlTemplates = fs.readdirSync(path.join(paths.keycloakThemesPath, themeDir, pagesDir, templatesDir)).filter(file => file.endsWith(".ftl"));
      const componentPages = fs.readdirSync(path.join(paths.keycloakThemesPath, themeDir, pagesDir, templatesDir)).filter(file => file.endsWith(".page.tsx"));
      if (ftlTemplates.length && componentPages.length) {
        ...
        keycloakTemplates.push({
          ...
          entry: {
            ...
            path: path.join(paths.keycloakThemesPath, themeDir, pagesDir, templatesDir, componentPage),
          }
        });
      }
    }
